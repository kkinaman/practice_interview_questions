...
var shuffleDeck = function(deck) {
  ...
  //   var indexToRemove = Math.round(Math.random() * i);
  //   shuffledDeck.push(deck[indexToRemove]);
  //   deck.splice(indexToRemove, 1);
  // }
  // return shuffledDeck;
  var numLeft = deck.length - 1;

  while (numLeft) {
    var randomPick = Math.round(Math.random() * numLeft);

    var temp = deck[randomPick];
    deck[randomPick] = deck[numLeft];
    deck[numLeft] = temp;
