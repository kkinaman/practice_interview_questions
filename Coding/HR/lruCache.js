...
LRUCache.prototype.get = function (key) {
  var value = null;
  if (this.items[key]) {
    value = this.items[key].val.val;
    this.cacheList.moveToFront(this.items[key]);
  }
  return value;
};
...
LRUCache.prototype.set = function (key, val) {
  ...
  var newListNode = this.cacheList.unshift(newItem);
  //store listNode in object (for constant lookup)
  this.items[key] = newListNode;
  //increment size
  this.cacheSize++;
  //if the list is full
  if (this.size() > this.limit) {
    //remove the last item (least recently used)
    var keyToRemove = this.cacheList.pop().key;
    console.log('REMOVING', keyToRemove);
    //remove from obj
    this.items[keyToRemove] = null;
    delete this.items.keyToRemove;
    this.cacheSize--;
  }
};
