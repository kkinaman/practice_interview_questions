...
var merge = function(arr1, arr2) {
  ...
  var cur2 = 0;
  var result = [];
  while (arr1[cur1] && arr2[cur2]) {
    if (arr1[cur1] <= arr2[cur2]) {
      while (arr1[cur1] && arr1[cur1] <= arr2[cur2]) {
        result.push(arr1[cur1]);
        cur1++;
      }
    } else {
      while (arr2[cur2] && arr2[cur2] < arr1[cur1]) {
        result.push(arr2[cur2]);
        cur2++;
      }
    }
  }
  //one of the arrays will still have contents, add those to results
  if (arr1[cur1]) {
    result = result.concat(arr1.slice(cur1));
  } else if (arr2[cur2]) {
    result = result.concat(arr2.slice(cur2));
  }
  return result;
};
...
var mergeSort = function(array) {
  ...
  var sorted = array.map(item => [item]);
  var intermediary = [];
  while (sorted.length > 1) {  
    // Sort adjacent arrays
    for (var i = 0; i < sorted.length; i += 2) {
      if (sorted[i + 1]) {
        intermediary.push(merge(sorted[i], sorted[i + 1]));
      } else {
        intermediary.push(sorted[i]);
      }
    }
    sorted = intermediary.slice();
    intermediary = [];
  }
  return sorted[0];
};
