...
BinaryHeap.prototype.insert = function (value) {
  ...
  var curIndex = this._heap.length - 1;
  var parentIndex = Math.floor( (curIndex - 1) / 2 );
  while (this._compare(this._heap[curIndex], this._heap[parentIndex])) {
...
//write helper functions for swap and findlesserchild
...
BinaryHeap.prototype.removeRoot = function () {
  ...
  // if there is anything left in the heap
  if (this._heap[0]) {
    var curIndex = 0;
    //find the smaller child of cur
    var smallerIndex = this.findLesserChildIndex(curIndex);
    // while there is a child and cur is not less than both children (i.e. less than the lesser child)
    while (smallerIndex && !this._compare(this._heap[curIndex], this._heap[smallerIndex])) {
      this.swap(curIndex, smallerIndex);
      curIndex = smallerIndex;
      smallerIndex = this.findLesserChildIndex(curIndex);
    }
  }
  return root;
}

BinaryHeap.prototype.findLesserChildIndex = function(index) {
  if (!this._heap) return null;
  var childrenIndices = [index * 2 + 1, index * 2 + 2].filter(i =>  i < this._heap.length );
  if (!childrenIndices.length) {
    return null;
  } else if (childrenIndices.length === 1) {
    return childrenIndices[0];
  } else {
    return (this._heap[childrenIndices[0]] < this._heap[childrenIndices[1]]) ? childrenIndices[0] : childrenIndices[1];
  }
}
